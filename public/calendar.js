document.addEventListener('DOMContentLoaded', function() {
    ...
    let currentDate = jalaali.toJalaali(new Date());
    let currentMonth = currentDate.jm;
    let currentYear = currentDate.jy;
    ...
    // Function to fetch tasks for a specific month
    function fetchTasksForMonth(month, year) {
        // Get the first day of the month
        const firstDay = jalaali.jalaaliToDateObject(year, month, 1);
        // Get the last day of the month
        const lastDay = jalaali.jalaaliToDateObject(year, month+1, 0);

        // Format dates for API
        const startDate = firstDay.toISOString().split('T')[0];
        const endDate = lastDay.toISOString().split('T')[0];

        fetch(`/api/tasks?start_date=${startDate}&end_date=${endDate}`, {
            method: 'GET',
            headers: {
    ...
    function generateCalendarDays(month, year) {
        ...
        const daysInMonth = getDaysDifference(firstDay,lastDay) + 1;
        console.log(firstDay,lastDay,daysInMonth,month)
        // Create a day element for each day in the month
        for (let day = 1; day <= daysInMonth; day++) {
            const date = jalaali.jalaaliToDateObject(year, month, day);
            const dyear = date.getFullYear();
            const dmonth = String(date.getMonth() + 1).padStart(2, '0'); // Months are 0-indexed
    ...
    function addMonthNavigation() {
        ...
        if (!document.querySelector('.calendar-navigation')) {
    ...
    function openTasksModal(dateTime, day, holiday) {
        ...
        const formattedDate = `${date.toLocaleString('default', { month: 'long' })} ${day}, ${date.getFullYear()}`;
